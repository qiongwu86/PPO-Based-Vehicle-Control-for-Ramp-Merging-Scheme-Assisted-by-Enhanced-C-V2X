...
namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("LtePhyErrorModel");
...
int16_t
LtePhyErrorModel::GetRowIndex (uint16_t mcs, uint8_t harq)
{
...
int16_t
LtePhyErrorModel::GetColIndex (double val, double min, double max, double step)
{
...
double
LtePhyErrorModel::GetBlerValue (const double (*xtable)[XTABLE_SIZE], const double (*ytable), const uint16_t ysize, uint16_t mcs, uint8_t harq, double sinr)
{
...
double
LtePhyErrorModel::GetSinrValue (const double (*xtable)[XTABLE_SIZE], const double (*ytable), const uint16_t ysize, uint16_t mcs, uint8_t harq, double bler)
{
  double sinr = 0;
  int16_t rIndex = GetRowIndex (mcs, harq);
  uint16_t index = 0;
...
double 
LtePhyErrorModel::GetValueForIndex (uint16_t index, double min, double max, double step)
{
...
uint16_t
LtePhyErrorModel::GetBlerIndex (double bler, uint16_t row, const double (*ytable), const uint16_t ysize)
{
  uint16_t index = 0;
...
TbErrorStats_t
LtePhyErrorModel::GetBler (const double (*xtable)[XTABLE_SIZE], const double (*ytable), const uint16_t ysize, uint16_t mcs, uint8_t harq, double prevSinr, double newSinr)
  {
...
TbErrorStats_t
LtePhyErrorModel::GetPsschBler (LteFadingModel fadingChannel, LteTxMode txmode, uint16_t mcs, double sinr, HarqProcessInfoList_t harqHistory)
{
  //Check mcs values
  if (mcs > 20) 
    {
...
TbErrorStats_t
LtePhyErrorModel::GetPsdchBler (LteFadingModel fadingChannel, LteTxMode txmode, double sinr, HarqProcessInfoList_t harqHistory)
{
  //Find the table to use
  const double (*xtable)[XTABLE_SIZE];
  const double *ytable;
...
TbErrorStats_t
LtePhyErrorModel::GetPscchBler (LteFadingModel fadingChannel, LteTxMode txmode, double sinr)
{
  //Find the table to use
  const double (*xtable)[XTABLE_SIZE];
  const double *ytable;
...
TbErrorStats_t
LtePhyErrorModel::GetPuschBler (LteFadingModel fadingChannel, LteTxMode txmode, uint16_t mcs, double sinr, HarqProcessInfoList_t harqHistory)
{
  //Check mcs values
  if (mcs > 28) 
    {
...
TbErrorStats_t
LtePhyErrorModel::GetPsbchBler (LteFadingModel fadingChannel, LteTxMode txmode, double sinr)
{
  //Find the table to use
  const double (*xtable)[XTABLE_SIZE];
  const double *ytable;
