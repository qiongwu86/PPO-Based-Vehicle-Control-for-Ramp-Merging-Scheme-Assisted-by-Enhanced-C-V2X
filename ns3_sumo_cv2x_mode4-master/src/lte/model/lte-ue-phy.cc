...
LteUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUePhy")
    ...
    .AddAttribute ("UeRandomInitialSubframeIndication",
                   ...
                   BooleanValue(false),
                   MakeBooleanAccessor(&LteUePhy::SetInitialSubFrameIndication),
                   MakeBooleanChecker())
    ...
    .AddTraceSource ("SidelinkV2xAnnouncement",
                     ...
                     MakeTraceSourceAccessor (&LteUePhy::m_sidelinkV2xAnnouncementTrace),
                     "ns3::LteUePhy::SidelinkV2xAnnouncementTracedCallback")
    //enable V2X
    .AddAttribute ("EnableV2x",
                   "If true, V2X transmission will be enabled.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&LteUePhy::m_v2xEnabled),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
//             NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
      ...
      else if (msg->GetMessageType() == LteControlMessage::SCI_V2X) // SCI-1 received
        {
          Ptr<SciLteControlMessageV2x> msg2 = DynamicCast<SciLteControlMessageV2x> (msg);
          SciListElementV2x sci1 = msg2->GetSci(); 
          ...
          if (m_ueSlssScanningInProgress)
            {
              ...
              m_detectedMibSl.insert (std::pair <std::pair<uint16_t,uint16_t>, LteRrcSap::MasterInformationBlockSL> (std::pair<uint16_t,uint16_t>(mibSL.slssid, mibSL.rxOffset), mibSL));
            }
        }
      //discovery
...
// 从m_sidelinkRxPoolsV2x读出信息，保存感知信息
      ...
      if (rbMask.size () == 0)
        {
          ...
          for (ctrlIt=ctrlMsg.begin() ; ctrlIt != ctrlMsg.end() && !sciDiscfound; ctrlIt++) {
              sciDiscfound = (*ctrlIt)->GetMessageType () == LteControlMessage::SCI || (*ctrlIt)->GetMessageType() == LteControlMessage::SCI_V2X || (*ctrlIt)->GetMessageType () == LteControlMessage::SL_DISC_MSG;
                  ...
                  if ((*ctrlIt)->GetMessageType () == LteControlMessage::DL_CQI
                  ...
                  else
                    {
                      NS_ASSERT ((*ctrlIt)->GetMessageType () == LteControlMessage::SCI
                                 || (*ctrlIt)->GetMessageType () == LteControlMessage::SCI_V2X
      ...
      if (rbMask.size() != 0 || (ctrlMsg.size () > 0 && (*ctrlMsg.begin())->GetMessageType () != LteControlMessage::SCI 
          && (*ctrlMsg.begin())->GetMessageType () != LteControlMessage::SCI_V2X
          ...
          else if(m_v2xEnabled)
            {
              ...
              if (ctrlMsg.size ()>0)
                { 
                  ...
                  else if ((*msgIt)->GetMessageType () == LteControlMessage::SCI_V2X)
                    {
                      NS_LOG_LOGIC (this << " UE - start TX PSCCH");
                      //access the control message to store the PSSCH grant and be able to
                      //determine the subframes/RBs for PSSCH transmissions
                      
                      NS_ASSERT_MSG ((*msgIt)->GetMessageType () == LteControlMessage::SCI_V2X, "Received " << (*msgIt)->GetMessageType ());

                      Ptr<SciLteControlMessageV2x> msg2 = DynamicCast<SciLteControlMessageV2x> (*msgIt);
                      SciListElementV2x sci1 = msg2->GetSci ();
                      ...
                      if (!mibSLfound)
                        {
                          if(m_ueSlssScanningInProgress)
                          ...
                          else
                            {
                              //SetSubChannelsForTransmission (slRb);
                              //m_uplinkSpectrumPhy->StartTxSlDataFrame (pb, ctrlMsg, UL_DATA_DURATION, 0);
                              m_uplinkSpectrumPhy->ChangeState (LteSpectrumPhy::State::TX_UL_V2X_SCI);
                            }
                        }
                      else
                        {
...
void
LteUePhy::DoSetSlTxPool (Ptr<SidelinkTxDiscResourcePool> pool)
{
...
void
LteUePhy::DoRemoveSlTxPool (bool disc)
{
  m_discTxPools.m_pool = NULL;
...
void
LteUePhy::DoSetSlRxPools (std::list<Ptr<SidelinkRxDiscResourcePool> > pools)
{
...
void
LteUePhy::DoSetSlTxPool (Ptr<SidelinkTxCommResourcePool> pool)
{
...
void 
LteUePhy::DoSetSlV2xTxPool (Ptr<SidelinkTxCommResourcePoolV2x> pool)
{
...
void
LteUePhy::DoSetSlRxPools (std::list<Ptr<SidelinkRxCommResourcePool> > pools)
{
...
void 
LteUePhy::DoSetSlV2xRxPools (std::list<Ptr<SidelinkRxCommResourcePoolV2x> > pools)
{
...
void LteUePhy::SetFirstScanningTime(Time t){
  NS_LOG_FUNCTION (this);
  m_tFirstScanning = t;
  Simulator::Schedule(m_tFirstScanning,&LteUePhy::StartSlssScanning, this);
}

Time LteUePhy::GetFirstScanningTime(){
  NS_LOG_FUNCTION (this);
  return m_tFirstScanning;
}

double LteUePhy::GetSidelinkRsrp (std::vector<SpectrumValue> p)
{
...
double LteUePhy::GetSidelinkRssi (std::vector<SpectrumValue> sig, std::vector<SpectrumValue> interference)
{
...
void LteUePhy::ReceiveSlss(uint16_t slssid, Ptr<SpectrumValue> p)
{
...
void LteUePhy::SetInitialSubFrameIndication(bool rdm)
{
  ...
  if (rdm)
    {
      ...
      Simulator::ScheduleNow(&LteUePhy::SubframeIndication, this,
...
void LteUePhy::SetUeSlssInterScanningPeriodMax(Time t)
{
  NS_LOG_FUNCTION (this);
  m_nextScanRdm->SetAttribute("Max",DoubleValue(t.GetMilliSeconds()) );
}

void LteUePhy::SetUeSlssInterScanningPeriodMin(Time t)
{
  NS_LOG_FUNCTION (this);
  m_nextScanRdm->SetAttribute("Min",DoubleValue(t.GetMilliSeconds()) );
}

void LteUePhy::StartSlssScanning()
{
  NS_LOG_FUNCTION (this);
...
void LteUePhy::StartSlssMeasurements(uint64_t slssid, uint16_t offset)
{
  NS_LOG_FUNCTION (this);
...
void LteUePhy::ReportSlssMeasurements(uint64_t slssid, uint16_t offset)
{
  NS_LOG_FUNCTION (this);
      ...
      for (it = m_ueSlssMeasurementsMap.begin(); it != m_ueSlssMeasurementsMap.end(); it++)
        {
          ...
          LteUeCphySapUser::UeSlssMeasurementReportElement newEl;
...
void LteUePhy::ScheduleNextSyncRefReselection(uint16_t endOfPrevious ){
  NS_LOG_FUNCTION (this);
...
void LteUePhy::DoSetSlssId(uint64_t slssid)
{
  NS_LOG_FUNCTION (this);
...
void LteUePhy::DoSendSlss(LteRrcSap::MasterInformationBlockSL mibSl)
{
  NS_LOG_FUNCTION (this);
...
void LteUePhy::DoSynchronizeToSyncRef(LteRrcSap::MasterInformationBlockSL mibSl)
{
  NS_LOG_FUNCTION (this);
